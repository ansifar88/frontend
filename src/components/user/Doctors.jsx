            ...
            <div className="grid sm:grid-cols-2 ">
                {data.data.map(
                    ({ name, currentHospital, _id, department, qualification, displaypicture }) => (
                        ...
                        <Card color="transparent" key={_id} shadow={false} className="w-72 h-28 md:w-full md:h-52 max-w-[45rem] bg-[#CAF0F8] mt-5 ms-5 hover:bg-[#84d0fb] cursor-pointer" onClick={() => navigate('/doctorview', { state: { _id } })}>
                            <CardHeader
                                color="transparent"
                                floated={false}
                                shadow={false}
